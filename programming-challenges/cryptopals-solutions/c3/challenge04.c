...
int main() {
  double_t line_current_score = 0.0;
  double_t line_decryption_score = MAX_SIZE;
    ...
    while (fgets(string, MAX_SIZE, file)) {
      ...
      if (string_size > 0) {
        ...
        if (buffer_size > 0) {
          ...
          if (output_size > 0) {
            line_current_score = evaluate_decryption(output, output_size);
            if (line_current_score < line_decryption_score) {
              line_decryption_score = line_current_score;
              line_size = output_size;
              memcpy(line, output, output_size);
            }
          }
        }
      }
