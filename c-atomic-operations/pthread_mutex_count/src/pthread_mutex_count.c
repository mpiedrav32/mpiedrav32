...
typedef struct thread_data {
  ...
  size_t cycle_count;
  size_t step_size;
  pthread_mutex_t can_increase_counter;
} thread_data_t;
...
void* accumulate(void* data) {
  ...
  for (size_t index = 0; index < shared_data->cycle_count; ++index) {
    pthread_mutex_lock(&shared_data->can_increase_counter);
    shared_data->counter += shared_data->step_size;
    pthread_mutex_unlock(&shared_data->can_increase_counter);
  }
...
void run_acc(size_t cycle_count, size_t step_size, size_t thread_count) {
  ...
  pthread_mutex_init(&shared_data->can_increase_counter, NULL);
  ...
  pthread_mutex_destroy(&shared_data->can_increase_counter);
